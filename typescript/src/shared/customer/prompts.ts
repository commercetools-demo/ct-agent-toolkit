...
This tool will create a new Customer in commercetools.
...
export const createCustomerInStorePrompt = `
This tool will create a new Customer in a specific store in commercetools.

It takes these required arguments:
- email (string): Customer email address.
- password (string): Customer password.
- storeKey (string): The key of the store to create the customer in.
...
export const getCustomerByIdPrompt = `
This tool will fetch a Customer by ID from commercetools.

It takes these required arguments:
- id (string): The ID of the customer to fetch.

It takes these optional arguments:
- expand (array of strings, optional): Fields to expand. Example: ["customerGroup"]
`;

export const getCustomerInStoreByIdPrompt = `
This tool will fetch a Customer by ID from a specific store in commercetools.

It takes these required arguments:
- id (string): The ID of the customer to fetch.
- storeKey (string): The key of the store to fetch the customer from.

It takes these optional arguments:
- expand (array of strings, optional): Fields to expand. Example: ["customerGroup"]
`;

export const queryCustomersPrompt = `
This tool will query Customers from commercetools.
