...
(async (): Promise<void> => {
  console.log('--- Starting Commercetools Langchain Agent Task Sequence (with Chat History) ---');
  ...
  type AgentResult = {
    ...
    intermediate_steps?: Array<{action: {tool: string, toolInput: any, log: string}, observation: string}>;
    [key: string]: any;
  };
  ...
  const recordAndLog = (promptText: string, agentResult: AgentResult) => {
    ...
    } else if (typeof agentResult.output === 'object' && agentResult.output !== null) {
    ...
    if (agentResult.intermediateSteps && agentResult.intermediateSteps.length > 0) {
      ...
      agentResult.intermediateSteps.forEach((step: any, index: number) => {
        ...
        console.log(`  Tool Input: ${typeof step.action.toolInput === 'string' ? step.action.toolInput : JSON.stringify(step.action.toolInput)}`);
  ...
  const initialPromptText = 'List all products in the commercetools project. This is the first step.';
  ...
  const prompt1Text = "List all product types available in the project. I will need this information to select a product type for creating a product in the future (not now).";
